...
Similarly to what we did previously in the **Webex Instant Connect API** section earlier, here we will create Webex Instant Connect meeting links.  However, instead of using Postman we will use an **HTTP Request** node. 

1. As we did in the **Webex Connect Flow: Reminder** section, we will create some Custom Variables
...
- To get the value for **Input: Import Data From**, we will do as we did in the **Webex Connect Flow: Reminder** section, choosing the `http.response.body` variable from the `Create Meeting Links` **HTTP Request** node configured in the previous step:

![Data Parser node setup](images/data-parser-ic.png)

> **Note:** Node numbers may be different.

- For the **Sample Body**, you can use the response body you got in Postman. 
